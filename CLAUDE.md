...
Expert skill for creating professional web animations using GSAP, Framer Motion (Motion), Anime.js, Animate.css, Magic Animations, and CSS native animations. Use this skill whenever the user asks to animate any web element, create transitions, add scroll-triggered effects, build micro-interaction...

- `.claude/skills/web-animation-expert/SKILL.md`
- `.claude/skills/web-animation-expert/references/animejs.md`: Anime.js v4 uses a modular import system — import only what you need for smaller bundles. The library is ~15kb minified (significantly lighter than GSAP's ~30kb or Framer Motion's ~30kb).
- `.claude/skills/web-animation-expert/references/css-animations.md`: Covers: **Animate.css**, **Magic Animations**, **Native CSS Transitions/Keyframes**, and **Tailwind CSS Animation Utilities**.
- `.claude/skills/web-animation-expert/references/framer-motion.md`: **Next.js App Router**: Components using `motion` need `"use client"` directive.
- `.claude/skills/web-animation-expert/references/gsap.md`: GSAP is now free for everyone (including commercial use) thanks to Webflow's sponsorship. All plugins are included.
